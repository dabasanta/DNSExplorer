...
basicRecon(){
  ...
  if [ -z "$2" ];then
    if host -t CNAME "$1" | grep 'alias' >/dev/null 2>&1;then
      ...
      echo -e " Hosts $1 has not alias records\n"
    fi
  else
  ...
  echo -e " Finding text description for $1 domain \e[92m"
  ...
  if [ -z "$2" ];then
    if host -t txt "$1" | grep 'descriptive' >/dev/null 2>&1;then
      ...
      echo -e " Hosts $1 has not description records\n"
    fi
  else
  ...
  echo -e " Checking if $1 has a TLS site\e[92m"
  ...
  if [[ "$connected" == "CONNECTED" ]];then
      ...
      if [[ "$DNS" == "DNS" ]];then
          echo -e " The domain $1 has a secure webserver and your certificate have these alternate domain names:\e[92m"
          ...
          echo -e " $subjects alternate DNS domain found.\n"
          crtSH "$1"
      else
          echo -e " Domain $1 has secure website at $1:443, but does not have alternate subject names.\n"
          crtSH "$1"
      fi
  else
      echo -e " No website found on $1:443\n"
  fi

  echo -e " Finding nameserver address for $1 domain \e[92m"
