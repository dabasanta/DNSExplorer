...
bruteForceDNS(){
    echo -e "[++] Fuzzing subdomains of $1\n[?] Do yo want to use a custom dictionary? [C=custom/d=Default]\n[?] Default: Provides a dictionary with the top 1000 of the most commonly used subdomains.\nCustom: Use your own custom dictionary.";while true; do read -rp "[D/c]> " dc;case $dc in
            [Dd]* ) dictionaryAttack "$1"; break;;
            [Cc]* ) dictionaryAttackCustom "$1"; break;;
            * ) echo -e "[!!] Please answer D or C\n";;esac;done
}
crtSH(){
  domain_search=$1;echo -e "\n$info Finding subdomains - abusing Certificate Transparency Logs using https://crt.sh/\n$end";curl -s "https://crt.sh/?q=${domain_search}&output=json" | jq .[].name_value | sed 's/"//g' | tr '\n' '\n';echo -e "\n"
}
basicRecon(){
    echo -e "[+] Finding IP address for A records";host "$1" | grep 'has address'|awk '{print $4}';echo -e "";echo -e "[+] Finding IPv6 address for AAA records";if host "$1" | grep 'IPv6' >/dev/null 2>&1;then host $1 | grep 'IPv6'| awk '{print $5}';echo -e "";else echo -e "[?] Hosts $1 has not IPv6 address\n";fi;echo -e "[+] Finding mail server address for $1 domain";if host -t MX "$1" | grep 'mail' >/dev/null 2>&1;then host "$1" | grep 'mail'|awk '{print $6,$7}';echo -e "";else echo -e "[?] Hosts $1 has not mail server records\n";fi;echo -e "[+] Finding CNAME records for $1 domain";if host -t CNAME "$1" | grep 'alias' >/dev/null 2>&1;then host -t CNAME "$1"|awk '{print $1,$4,$6}';echo -e "";else echo -e "[?] Hosts $1 has not alias records\n";fi;echo -e "[+] Finding text description for $1 domain";if host -t txt "$1" | grep 'descriptive' >/dev/null 2>&1;then host -t txt "$1" | grep 'descriptive';echo -e "";else echo -e "[?] Hosts $1 has not description records\n";fi;echo "[+] Checking if $1 has a TLS site";connected=$(echo -n|openssl s_client -connect  "$1:443" 2>/dev/null|head -1|awk -F "(" '{print $1}');if [[ $connected == "CONNECTED" ]];then DNS=$(echo -n|openssl s_client -connect "$1:443" 2>/dev/null|sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p'|openssl x509 -text | sed 's/\                //'|grep -i "DNS:"|awk -F ":" '{print $1}');if [[ $DNS == "DNS" ]];then echo "[++]The domain $1 has a secure webserver and your certificate have these alternate domain names:";echo -n |openssl s_client -connect "$1:443" 2>/dev/null| sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' | openssl x509 -text | grep "DNS:"| tr ',' '\n'|sed 's/\               //'|sed 's/\s//g'|sed 's/DNS://g';subjects=$(echo -n |openssl s_client -connect "$1:443" 2>/dev/null| sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' | openssl x509 -text | grep "DNS:"| tr ',' '\n'|sed 's/\               //'|wc -l);echo -e "[++]$subjects alternate DNS domain found.\n";crtSH "$1";else echo -e "[?] Domain $1 has secure website at $1:443, but does not have alternate subject names.\n";crtSH "$1";fi;else echo -e "$error No website found on $1:443\n";fi;echo -e "[+] Finding nameserver address for $1 domain";if host -t NS "$1" | grep 'name server' >/dev/null 2>&1;then host -t NS "$1" | cut -d " " -f 4;host -t NS "$1" | cut -d " " -f 4 > /tmp/dnsexplorer/NameServers.txt;ns=$(wc -l /tmp/dnsexplorer/NameServers.txt | awk '{print $1}');echo -e "\n $ns DNS Servers was found, trying ZoneTransfer on these servers";if doZoneTransfer "$1";then echo -e "\nDNS zone transfer was possible, no bruteforce attacks on the subdomains are required.\n";clean;else echo -e "\n[!!] DNS zone transfer was not possible, DNS servers are not accept it";while true; do echo "";read -rp "Do you want to brute force subdomains? [Y/n]> " yn
    case $yn in
